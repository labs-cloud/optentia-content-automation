/**
 * Seed + backfill core, shared by:
 *  - scripts/seed.ts (local `pnpm seed`)
 *  - the cron-triggered runtime migration (cronHandlers.ts) so a fresh deploy
 *    bootstraps itself without a local dev environment.
 *
 * Everything here is idempotent (looked up by name before insert).
 */
import {
  backfillClientId,
...
export async function resolveOwnerUserId(): Promise<number> {
...
export async function ensureOptentia(userId: number) {
...
export async function ensureDemoClient(userId: number) {
...
/** Full bootstrap: schema columns are assumed present (ensureMultiClientSchema). */
export async function runSeed(): Promise<{ ownerUserId: number }> {
